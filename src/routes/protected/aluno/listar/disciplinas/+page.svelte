<script lang="ts">
    ...
    const abrirModal = (studentData: unknown) => {
      turmaSelecionada = studentData;
      showModal = true;
    };
    ...
    {#if studentData > 0}
      <div class="grid gap-4 grid-cols-3 m-10 mx-0">
        ...
        <h2 class="text-3xl font-semibold text-primary mb-2">
            Turma: {studentData.identificador}
        </h2>
        <p class="text-xl font-medium">Sala: {studentData.sala}</p>
        ...
        <button
            ...
            on:click={() => abrirModal(studentData)}
        >
            Ver mais
        </button>
        </div>
