...
<div class="mt-12 ml-10 mr-10">
      ...
      <div class="grid gap-4 grid-cols-3 m-10 mx-0">
          {#each turmas as turma (turma.id)}
              <div class="bg-gray-200 shadow-no-blur-sm rounded-lg p-8 flex flex-col justify-between h-56" id="card">
                  <h2 class="text-3xl font-semibold text-primary mb-2">Turma: {turma.identificador}</h2>
                  <p class="text-xl font-medium">Sala: {turma.sala}</p>
                  <p class="text-xl font-medium">Temporada Letiva: {turma.temoporadaLetiva}</p>
  ...
  {:else}
    <div class="flex flex-col justify-center items-center h-64 bg-gray-100 rounded-lg shadow-md mt-8">
      ...
      <p class="text-2xl text-center text-gray-600">Ainda não há turmas cadastradas</p>
    </div>
  {/if}
      ...
      <div class="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
          <div class="bg-white rounded-lg p-8 w-4/5 md:w-1/2">
              <div class="flex justify-between">
                  <h2 class="text-2xl font-semibold text-primary">Detalhes da Turma: {turmaSelecionada.identificador}</h2>
                  <button
