...
export const actions: Actions = {
  login: async ({ cookies, request }: RequestEvent) => {
    ...
    // verificando o tempo de expiração do token:
    const seconds = Math.floor((maxAgeInMs / 1000) % 60);
    const minutes = Math.floor((maxAgeInMs / (1000 * 60)) % 60);
    const hours = Math.floor((maxAgeInMs / (1000 * 60 * 60)) % 24);
    const days = Math.floor(maxAgeInMs / (1000 * 60 * 60 * 24));

    console.log(
      `The token will expire in ${days} days, ${hours} hours, ${minutes} minutes, and ${seconds} seconds.`,
    );
